                ...
                <div class="p-6 text-gray-900 dark:text-gray-200">
                    ...
                    <form method="POST" action="{{ route('cotizaciones.update', $cotizacion->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-datepicker id="vigencia" class="block w-full" name="vigencia" :value="$cotizacion->vigencia" />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @foreach ($productos as $producto)
                                <div class="flex items-center mb-2">
                                    <input type="checkbox" id="producto_{{ $producto->id }}" name="productos[]" value="{{ $producto->id }}" class="hidden peer" {{ in_array($producto->id, $productos_cotizacion) ? 'checked' : '' }}>
                                    ...
                                    <input id="cantidad_{{ $producto->id }}" class="ml-4 w-1/3   form-input" type="number" name="cantidades[{{ $producto->id }}]" value="{{ $cantidades[$producto->id] ?? '' }}" placeholder="{{ __('Cantidad') }}" />
                                </div>
                            @endforeach
                        </div>
